...
function Step({ parent, children }) {
    return (
        <div className="w-5/12 flex flex-wrap justify-center text-center">
            <Link to={parent.path} className="basis-5/12 py-3 bg-indigo-200 border border-indigo-600 rounded-md">{parent.content}</Link>
            <div className="basis-full flex justify-center mb-2">
                <FontAwesomeIcon icon="caret-down" className="text-indigo-500 text-2xl" />
            </div>
            <ul className="basis-10/12 flex gap-1 p-1 border-2 border-indigo-600 rounded-md">
                {
                    children.map(item =>
