...
/** @private */
function sendMessage(logger, transportName, httpClient, url, accessTokenFactory, content, options) {
        ...
        return __generator(this, function (_c) {
            switch (_c.label) {
                ...
                case 1:
                    ...
                    if (token) {
                        headers = (_b = {},
                            _b["Authorization"] = "Bearer ".concat(token),
                            _b);
                    }
                    _c.label = 2;
                case 2:
                    ...
                    headers[name] = value;
                    logger.log(ILogger_1.LogLevel.Trace, "(".concat(transportName, " transport) sending data. ").concat(getDataDetail(content, options.logMessageContent), "."));
                ...
                case 3:
                    response = _c.sent();
                    logger.log(ILogger_1.LogLevel.Trace, "(".concat(transportName, " transport) request complete. Response status: ").concat(response.statusCode, "."));
                    return [2 /*return*/];
            }
        });
    });
...
var ConsoleLogger = /** @class */ (function () {
    ...
    ConsoleLogger.prototype.log = function (logLevel, message) {
        if (logLevel >= this._minLevel) {
            var msg = "[".concat(new Date().toISOString(), "] ").concat(ILogger_1.LogLevel[logLevel], ": ").concat(message);
            switch (logLevel) {
